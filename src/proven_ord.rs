...
pub trait ProvenOrd: OrdSpec + ProvenPartialOrd {
    ...
    proof fn proof_totality()
        ensures
            forall|x: &Self, y: &Self| #[trigger] x.partial_cmp_spec(y) == Some(x.cmp_spec(y)),
    ;
    ...
    proof fn proof_cmp_transitivity_lt()
        ensures
            forall|x: &Self, y: &Self, z: &Self|
                #![trigger x.cmp_spec(y), y.cmp_spec(z)]
                (x.cmp_spec(y) == Ordering::Less && y.cmp_spec(z) == Ordering::Less) ==> x.cmp_spec(
                    z,
                ) == Ordering::Less,
    ;
...
pub broadcast proof fn lemma_consistency_with_eq<T: ProvenOrd>(x: &T, y: &T)
    ensures
        (#[trigger] x.cmp_spec(y) == Ordering::Equal) <==> x.eq_spec(y),
{
