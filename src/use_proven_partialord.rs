...
verus! {
    ...
    pub fn compare<T: ProvenPartialOrd>(a: &T, b: &T) -> (result: Option<Ordering>)
        ...
        ensures result == a.partial_cmp_spec(b)
    {
        PartialOrd::partial_cmp(a, b)
    }

    pub fn duality_example<T: ProvenPartialOrd>(a: &T, b: &T)
        requires a.partial_cmp_spec(b) == Some(Ordering::Less)
        ensures b.partial_cmp_spec(a) == Some(Ordering::Greater)
    {
    }
    ...
    pub fn transitivity_example<T: ProvenPartialOrd>(a: &T, b: &T, c: &T)
        ...
        ensures a.partial_cmp_spec(c) == Some(Ordering::Less)
    {
    }
